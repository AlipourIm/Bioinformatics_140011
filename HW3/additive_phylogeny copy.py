...
def limb_length_calculator(distance_matrix, n_leaves, j_index):
    ...
    limb_length_calculator = min(limb_length_amount) # Min of these limb length is our answer.
...
def additive_phylogeny_algorithm(distance_matrix, number_of_leaves, graph, int_node):
    ...
    while int_node in list(graph.nodes):
        int_node += 1
    recursive_graph = additive_phylogeny_algorithm(distance_matrix, number_of_leaves - 1, graph, int_node)

    number_of_vertices = -1
    source_path = networkx.shortest_path(recursive_graph, source=selected_i_node, target=selected_k_node)    # Find shortest path between source to target in new matrix
    distance = 0
    for j in range(1, len(source_path) - 1):
        distance += recursive_graph[source_path[j - 1]][source_path[j]]['weight']
        if distance == x:
            number_of_vertices = source_path[j]

    if number_of_vertices == -1:
        number_of_vertices = int_node
        while number_of_vertices in list(recursive_graph.nodes):
            number_of_vertices += 1
        distance = 0
        j = 0
        while distance < x:
            ...
            pdist = distance
            distance += recursive_graph[source_path[j - 1]][source_path[j]]['weight']
        recursive_graph.remove_edge(source_path[j - 1], source_path[j])
        recursive_graph.add_edge(number_of_vertices, source_path[j], weight=distance - x)
        recursive_graph.add_edge(number_of_vertices, source_path[j - 1], weight=x - pdist)

    recursive_graph.add_edge(number_of_vertices, new_n, weight=limb_length_here)
...
graph = networkx.Graph()  # Create an empty graph
result = additive_phylogeny_algorithm(distance_matrix, new_n, graph, new_n)
adj_dict = networkx.to_dict_of_lists(result)  # Convert the result to a dictionary
array = adj_dict.items()
answer = []
